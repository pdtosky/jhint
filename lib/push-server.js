...
async function verifyPushNotificationUser(request) {
  ...
  if (!PUSH_NOTIFICATION_ROLES.has(role)) {
    return { ok: false, status: 403, message: "생산 또는 관리자 계정만 작업 알림을 설정할 수 있습니다." };
  }

  const registeredDisplayName = String(
    user.user_metadata?.display_name || user.user_metadata?.name || user.user_metadata?.full_name || ""
  ).trim();
  if (role === "production" && !registeredDisplayName) {
    return { ok: false, status: 400, message: "계정에 작업자 이름이 등록되어 있지 않습니다." };
  }
  const displayName = registeredDisplayName || "관리자";
